...
public class MatchRegistry {
  ...
  public <T> Optional<T> get(Class<T> clazz, String id, boolean required)
      throws MatchRegistryException {
    RegisterableObject found = this.registry.get(id);

    if (found == null) {
      if (required) {
        throw new MatchRegistryException("Could not find required " + clazz.getSimpleName()
            + " with id " + id + " in MatchRegistry");
      }
    ...
    throw new MatchRegistryException(
        "Mismatch! For id " + id + ": Expected to find " + clazz.getSimpleName() + " but found "
            + found.getObject().getClass().getSimpleName() + " instead.");
  }
}
