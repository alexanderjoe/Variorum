...
public class TeamsModule implements Module {
  ...
  public void setPlayerTeam(Player player, Team team) {
    ...
    if (scoreboardTeam == null) {
      ...
      scoreboardTeam.setOption(
          org.bukkit.scoreboard.Team.Option.NAME_TAG_VISIBILITY, OptionStatus.ALWAYS);
  ...
  public boolean canJoinTeam(Team team) {
    long teamSize = playerTeams.values().stream().filter(t -> t.equals(team)).count();

    long minTeamSize = teams.stream()
        .mapToLong(t -> playerTeams.values().stream().filter(pt -> pt.equals(t)).count())
        .min()
        .orElse(0);

    // Allow joining if this would not make the team more than 1 player larger than the smallest
    // team
  ...
  public Team autoAssignTeam(Player player) {
    return teams.stream()
        .min(Comparator.comparingLong(
            team -> playerTeams.values().stream().filter(t -> t.equals(team)).count()))
        .orElse(null);
  }
}
