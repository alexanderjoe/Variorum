...
public class Countdown {
   ...
   */
  public Countdown(
      Match match, int seconds, CountdownOptions options, Consumer<Countdown> onComplete) {
    this.match = match;
  ...
  public void start() {
    ...
    task =
        Variorum.get().getServer().getScheduler().runTaskTimer(Variorum.get(), this::tick, 0L, 20L);
  }
    ...
    if (options.isTitlesEnabled() && seconds <= 5) {
      match.getWorld().getPlayers().forEach(player -> {
        player.showTitle(Title.title(
            ...
            Component.empty(),
            Title.Times.times(
                Duration.ofMillis(0), Duration.ofMillis(1000), Duration.ofMillis(0))));
      });
    }
  }
}
